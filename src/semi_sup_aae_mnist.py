...
def convert_one_hot_encode(x_inp, n_labels):
    '''
    Specifically used for the SVHN labels to convert them to one hot encoding
    :param x_inp:
    :param n_labels:
    :return:
    '''

    x_out = np.zeros((x_inp.shape[0], n_labels))
        ...
        if train_model:
            ...
            for i in range(n_epochs):
                ...
                for b in range(1, n_batches + 1):
                    ...
                    batch_y_one_hot = convert_one_hot_encode(batch_y_l, 10)
                    ...
                    sess.run(supervised_encoder_optimizer, feed_dict={x_input_l: batch_x_l, y_input: batch_y_one_hot})
                    if b % 5 == 0:
                        a_loss, d_g_loss, d_c_loss, g_loss, s_loss, summary = sess.run(
                            [autoencoder_loss, dc_g_loss, dc_c_loss, generator_loss, supervised_encoder_loss,
                             summary_op],
                            feed_dict={x_input: batch_x_ul, x_target: batch_x_ul,
                                       real_distribution: z_real_dist, y_input: batch_y_one_hot, x_input_l: batch_x_l,
                                       categorial_distribution: real_cat_dist})
                        ...
                        # z_out = sess.run(encoder_output_latent, feed_dict={x_input: batch_x_l})
                        # batch_y_l_val = np.array([np.where(r == 1)[0][0] for r in batch_y_one_hot])
                        #
