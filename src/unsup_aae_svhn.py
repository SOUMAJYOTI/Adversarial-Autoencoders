...
def train(train_model=True):
    ...
    with tf.Session() as sess:
        if train_model:
            ...
            batch_y = np.array([], dtype=np.float32).reshape(0, 1)
            ...
            for i in range(n_epochs):
                ...
                for b in range(1, n_batches + 1):
                    ...
                    if b % 5 == 0:
                        a_loss, d_g_loss, d_c_loss, g_loss, summary = sess.run(
                            # [autoencoder_loss, dc_g_loss, dc_c_loss, generator_loss, supervised_encoder_loss,
                            #  summary_op],
                            [autoencoder_loss, dc_g_loss, dc_c_loss, generator_loss, summary_op],
                            feed_dict={x_input: batch_x_ul, x_target: batch_x_ul,
                        ...
                        # z_out = sess.run(z, feed_dict={x_input: batch_x_l})
                        # batch_y_l_val = np.expand_dims(batch_y_l, axis=1)
                        # z_out_final = np.vstack([z_out_final, z_out])
                        # batch_y = np.vstack([batch_y, batch_y_l_val])
                        #
                        # if i == 1:
                        #     plot_latent_2d(np.array(z_out_final), target=batch_y)
